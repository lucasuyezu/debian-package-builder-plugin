...
public class DebianPackagePublisher extends Recorder implements Serializable {
    ...
    public DebianPackagePublisher(String repoId, String commitMessage, boolean commitChanges, boolean republish,
            String republishDistribution) {
    ...
    public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener) throws IOException {
        ...
        if (republish) {
            ...
            if (build.getResult().isWorseThan(Result.SUCCESS)) {
                logger.println(MessageFormat.format(DebianPackageBuilder.ABORT_MESSAGE, PREFIX,
                        "Build is not success, will not execute debrelease"));
                return true;
            }
    ...
    private String getExpandedCommitMessage(AbstractBuild<?, ?> build, BuildListener listener) throws IOException,
    ...
    private void commitToGitAndPush(final AbstractBuild<?, ?> build, final Runner runner, GitSCM scm, String commitMessage)
            throws DebianizingException {
        try {
            GitCommitHelper helper = new GitCommitHelper(build, scm, runner, commitMessage,
                    DebianPackageBuilder.getRemoteModules(build));
    ...
    private void commitToSVN(final AbstractBuild<?, ?> build, final Runner runner, SubversionSCM svn, String commitMessage)
            throws DebianizingException {
        hudson.scm.SubversionSCM.DescriptorImpl descriptor = (hudson.scm.SubversionSCM.DescriptorImpl) Jenkins.getInstance()
                .getDescriptor(hudson.scm.SubversionSCM.class);
        ...
        try {
            for (String module : DebianPackageBuilder.getRemoteModules(build)) {
                ...
                runner.announce("Commited revision <{0}> of <{2}> with message <{1}>", runner.getChannel().call(helper),
                        commitMessage, module);
            }
        } catch (IOException e) {
            e.printStackTrace();
